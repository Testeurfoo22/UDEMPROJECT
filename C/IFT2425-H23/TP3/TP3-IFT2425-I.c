...
float trapeze(int NBINTERV){
    float X = (b-a)/NBINTERV;
    float sum = (Eq1(a) + Eq1(b))/2;
    for (int i = 1; i < NBINTERV; i++) {
        sum += Eq1(a + i*X);
    }
    //float quadraError = -(pow((b-a), 3)/(12*pow(NBINTERV, 2))) * -((4)/(pow((1-pow(0.0, 2)), 3/2)));
    sum = X*sum;
    //sum += quadraError;
    return sum;
}
void arrayCreate(float *flottant, int size){
    float X = (b-a)/size;
    flottant[0] = X*(Eq1(a)/2);
    for (int i = 1; i <= size-1; i++) {
        flottant[i] = X*Eq1(a + i*X);
    }
    flottant[size-1] = X*(Eq1(b)/2);
}
float divise(float *flottant, int size){
    float value = 0.0;
    ...
    if (size == 2){
        return (flottant[0] + flottant[1]);
    }
    int mid = (size/2);
    float *leftFlot = (float*)malloc((mid) * sizeof(float));
    float *rightFlot = (float*)malloc((size - mid) * sizeof(float));
    if (leftFlot == NULL || rightFlot == NULL) {
        printf("Failed to allocate memory!\n");
        return 1;
    }
    for (int i = 0; i < mid; i++) {
        leftFlot[i] = flottant[i];
    }
    for (int i = mid; i < size; i++) {
        rightFlot[i - mid] = flottant[i];
    }

    value += divise(leftFlot, mid);
    value += divise(rightFlot, size - mid);
...
float kahanFunct(float *flottant, int size){
    float s = 0.0;
    float e = 0.0;
    for (int x = 0; x < size; x++){
        float temp = s;
        float y = flottant[x] + e;
        s = temp + y;
        e = (temp - s) + y;
    }
    return s;
...
//--------------------------------------------------------------------------------
    ...
    //Programmer ici
    float piFind, Error, logEr;

    piFind = trapeze(NBINTERV);
    Error = fabs(piFind-PI);
    logEr = log10(Error);;
    printf("[1>Given_Order:]  Pi=%.10f Er=%.10f logEr=%.2f\n", piFind, Error, logEr);

    float *vectFlot = (float*) malloc(NBINTERV * sizeof(float));
    if (vectFlot == NULL) {
        printf("Failed to allocate memory!\n");
        return 1;
    }
    arrayCreate(vectFlot, NBINTERV);

    piFind = divise(vectFlot, NBINTERV);
    Error = fabs(piFind-PI);
    logEr = log10(Error);;
    printf("[2>PairwiseSum:]  Pi=%.10f Er=%.10f logEr=%.2f\n", piFind, Error, logEr);

    piFind = kahanFunct(vectFlot, NBINTERV);
    Error = fabs(piFind-PI);
    logEr = log10(Error);;
