...
void iDerivate(float** Img1, float** Img2, float** Ix, float** Iy, float** It, int length, int width){
    int sValue;
    for (int x = 1; x < length-1; x++){
        for (int y = 1; y < width-1; y++){
            /**secondValue = (Img2[(x)%length][(y+1)%width] - Img2[(x)%length][(y)%width] +
                            Img2[(x+1)%length][(y+1)%width] - Img2[(x+1)%length][(y)%width]);
            Ix[x][y] = (Img1[(x)%length][(y+1)%width] - Img1[(x)%length][(y)%width] +
                        Img1[(x+1)%length][(y+1)%width] - Img1[(x+1)%length][(y)%width] +
                        secondValue)/(4.0);
            Iy[x][y] = (Img1[(x+1)%length][(y)%width] - Img1[(x)%length][(y)%width] +
                        Img1[(x+1)%length][(y+1)%width] - Img1[(x)%length][(y+1)%width] +
                        secondValue)/(4.0);
            It[x][y] = (Img2[(x)%length][(y)%width] - Img1[(x)%length][(y)%width] +
                        Img2[(x+1)%length][(y+1)%width] - Img1[(x+1)%length][(y)%width] +
                        secondValue)/(4.0);**/
            Ix[x][y] = (Img1[x][y+1] - Img1[x][y] + Img1[x+1][y+1] - Img1[x+1][y] +
                        Img2[x][y+1] - Img2[x][y] + Img2[x+1][y+1] - Img2[x+1][y])/(4.0);
            Iy[x][y] = (Img1[x+1][y] - Img1[x][y] + Img1[x+1][y+1] - Img1[x][y+1] +
                        Img2[x+1][y] - Img2[x][y] + Img2[x+1][y+1] - Img2[x][y+1])/(4.0);
            It[x][y] = (Img2[x][y]   - Img1[x][y] + Img2[x+1][y] - Img1[x+1][y] +
                        Img2[x][y+1] - Img1[x][y+1] + Img2[x+1][y+1] - Img1[x+1][y+1])/(4.0);
        }
    }
}

float meanVector(float** v, int x, int y){
    return ((v[x-1][y]+v[x+1][y]+v[x][y+1]+v[x][y-1])/(6.0)) +
            ((v[x-1][y-1]+v[x-1][y+1]+v[x+1][y+1]+v[x+1][y-1])/(12.0));
}

void opticalCreator(float*** OptFl_Vx, float*** OptFl_Vy, float** VxM, float** VyM,
                    float** Ix, float** Iy, float** It, float alpha, int length, int width, int NBITER){
    for (int k = 1; k < NBITER; k++){
        for (int x = 5; x < length-5; x++){
            for(int y = 5; y < width-5; y++){
                float mVx = meanVector(OptFl_Vx[k-1], x, y);
                float mVy = meanVector(OptFl_Vy[k-1], x, y);
                OptFl_Vx[k][x][y] = mVx - ((Ix[x][y]*(Ix[x][y]*mVx + Iy[x][y]*mVy + It[x][y]))
                                    /(pow(alpha,2) + pow(Ix[x][y],2) + pow(Iy[x][y],2)));
                OptFl_Vy[k][x][y] = mVy - ((Iy[x][y]*(Ix[x][y]*mVx + Iy[x][y]*mVy + It[x][y]))
                                    /(pow(alpha,2) + pow(Ix[x][y],2) + pow(Iy[x][y],2)));
            }
        }
    }
...
int main(int argc,char** argv)
{
 ...
 //Programmer ici ........
 iDerivate(Img1, Img2, Ix, Iy, It, length, width);
 opticalCreator(OptFl_Vx, OptFl_Vy, VxM, VyM, Ix, Iy, It, alpha, length, width, NBITER);
